...
if {$composite} {
	...
	set centrex10 [expr -$f_x-0.5*$tw +0.5*$plt] 
	set centrey10 [expr -0.5*$h + 0.5*$tf]
	
	set centrex11 [expr -$f_x-0.5*$tw +0.5*$plt] 
	set centrey11 0.0

	set centrex12 [expr -$f_x-0.5*$tw +0.5*$plt] 
	set centrey12 [expr 0.5*$h - 0.5*$tf]	

	set centrex13 [expr $f_x+0.5*$tw+0.5*$plt] 
	set centrey13 [expr -0.5*$h + 0.5*$tf]
	
	set centrex14 [expr $f_x+0.5*$tw -0.5*$plt] 
	set centrey14 0.0

	set centrex15 [expr $f_x+0.5*$tw -0.5*$plt]
	set centrey15 [expr 0.5*$h - 0.5*$tf]
}
...
if {$protected} {
	...
	if {$stiffened} {
		...
		set centrex101 [expr -$f_x-0.5*$tw +0.5*$plt]  
		set centrey101 [expr -0.5*$h - $tf -0.5*$dp]
		
		set centrex102 [expr -$f_x-0.5*$tw - $plt - 0.5*$dp] 
		set centrey102 [expr -0.5*$h + 0.5*$tf]
		
		set centrex112 [expr -$f_x-0.5*$tw - $plt - 0.5*$dp] 
		set centrey112 0.0

		set centrex122 [expr -$f_x-0.5*$tw - $plt - 0.5*$dp] 
		set centrey122 [expr 0.5*$h - 0.5*$tf]	
		
		set centrex124 [expr -$f_x-0.5*$tw +0.5*$plt] 
		set centrey124 [expr 0.5*$h + $tf + 0.5*$dp]

		#right stiffening plate
		set centrex131 [expr $f_x+0.5*$tw+0.5*$plt] 
		set centrey131 [expr -0.5*$h - $tf -0.5*$dp]
		
		set centrex133 [expr $f_x+ 0.5*$tw + $plt + 0.5*$dp] 
		set centrey133 [expr -0.5*$h + 0.5*$tf]
		
		set centrex143 [expr $f_x+ 0.5*$tw + $plt + 0.5*$dp]
		set centrey143 0.0

		set centrex153 [expr $f_x+ 0.5*$tw + $plt + 0.5*$dp]
		set centrey153 [expr 0.5*$h - 0.5*$tf]	

		set centrex154 [expr $f_x+0.5*$tw -0.5*$plt]
		set centrey154 [expr 0.5*$h + $tf + 0.5*$dp]	
	} elseif {!$stiffened} {
...
if {$stiffened} {
	#left stiffening plate
	HTEntity Block 10 $centrex10 $centrey10 $plt $tf;
	HTEntity Block 11 $centrex11 $centrey11 $plt $w_y;
	HTEntity Block 12 $centrex12 $centrey12 $plt $tf;

	#right stiffening plate
	HTEntity Block 13 $centrex13 $centrey13 $plt $tf;
	HTEntity Block 14 $centrex14 $centrey14 $plt $w_y;
	HTEntity Block 15 $centrex15 $centrey15 $plt $tf;
}
...
if {$protected} {
	if {!$stiffened} {
		...
		#left stiffening plate
		HTEntity Block 101 $centrex101 $centrey101 $plt $dp;
		HTEntity Block 102 $centrex102 $centrey102 $dp $tf;
		HTEntity Block 112 $centrex112 $centrey112 $dp $w_y;
		HTEntity Block 122 $centrex122 $centrey122 $dp $tf;
		HTEntity Block 124 $centrex124 $centrey124 $plt $dp;

		#right stiffening plate
		HTEntity Block 131 $centrex131 $centrey131 $plt $dp;
		HTEntity Block 133 $centrex133 $centrey133 $dp $tf;
		HTEntity Block 143 $centrex143 $centrey143 $dp $w_y;
		HTEntity Block 153 $centrex153 $centrey153 $dp $tf;
		HTEntity Block 154 $centrex154 $centrey154 $plt $dp;
	}
	#bottom flange
...
# HTMesh $meshTag $EntityTag $MaterialTag 
  ...
  if {$stiffened} {
	HTMesh 10 		  10 		 1 -phaseChange 1 -NumCtrl $w_elemx $f_elemy
	HTMesh 11 		  11 		 1 -phaseChange 1 -NumCtrl $w_elemx $w_elemy
	HTMesh 12 		  12 		 1 -phaseChange 1 -NumCtrl $w_elemx $f_elemy

	HTMesh 13 		  13 		 1 -phaseChange 1 -NumCtrl $w_elemx $f_elemy
	HTMesh 14 		  14 		 1 -phaseChange 1 -NumCtrl $w_elemx $w_elemy
	HTMesh 15 		  15 		 1 -phaseChange 1 -NumCtrl $w_elemx $f_elemy
  }
...
if {$protected} {  
	if {!$stiffened} {
		...
		HTMesh 101 		  101 		 2 -phaseChange 0 -NumCtrl $w_elemx $p_elem
		HTMesh 102 		  102 		 2 -phaseChange 0 -NumCtrl $p_elem $f_elemy
		HTMesh 112 		  112 		 2 -phaseChange 0 -NumCtrl $p_elem $w_elemy
		HTMesh 122 		  122 		 2 -phaseChange 0 -NumCtrl $p_elem $f_elemy	
		HTMesh 124 		  124 		 2 -phaseChange 0 -NumCtrl $w_elemx $p_elem

		HTMesh 131 		  131 		 2 -phaseChange 0 -NumCtrl $w_elemx $p_elem
		HTMesh 133 		  133 		 2 -phaseChange 0 -NumCtrl $p_elem $f_elemy
		HTMesh 143 		  143 		 2 -phaseChange 0 -NumCtrl $p_elem $w_elemy
		HTMesh 153 		  153 		 2 -phaseChange 0 -NumCtrl $p_elem $f_elemy	
		HTMesh 154 		  154 		 2 -phaseChange 0 -NumCtrl $w_elemx $p_elem
	}
	#bottom flange
